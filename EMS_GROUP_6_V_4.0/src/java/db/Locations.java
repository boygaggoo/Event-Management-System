...
public class Locations {
    ...
    public static boolean addLocation(String buildingAbbrev, String fullName, String address, String roomNumber) {
    ...
    public static List<String> getLocations() {
        ...
        try {
            ...
            conn = DriverManager.getConnection(url);
            String cmd = "Select LOCATION_ID, BUILDING FROM LOCATION";
            Statement stat = conn.createStatement();
            ResultSet rs = stat.executeQuery(cmd);

            while (rs.next()) {
                
                String building = rs.getString(1) + " " + rs.getString(2);
    ...
    public static Location getLocation(int location_id) {
        ...
        try {
            conn = DriverManager.getConnection(url);
            String cmd = "SELECT * FROM LOCATION WHERE location_id =" + location_id;
            Statement stat = conn.createStatement();
            ResultSet rs = stat.executeQuery(cmd);
    ...
    public static boolean removeLocation(Location l) {
        boolean success = false;
        int rowsUpdated = 0;
        try {

            conn = DriverManager.getConnection(url);
            String location_id = l.getId();
            int iLocation_id = Integer.parseInt(location_id);
            String cmd = "DELETE FROM EMS.RESERVATION WHERE LOCATION_ID =" + iLocation_id;

            Statement stat = conn.createStatement();
            rowsUpdated = stat.executeUpdate(cmd);
            
            cmd = "DELETE FROM EMS.LOCATION WHERE LOCATION_ID =" + iLocation_id;
